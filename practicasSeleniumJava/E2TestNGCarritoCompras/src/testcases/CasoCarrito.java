...
public class CasoCarrito extends Base {
	...
	public void elegirArticulosAzarCarritoCompras() {
		...
		listaProductos = repoHomePage.listaElementos();
	...
	public void anadirProductosAlCarrito() {
		for(int i= 0 ; i < cantidadLista; i++ ) {
			...
			repoHomePage.addToCartElement(listaProductos.get(productosAlAzar.get(i) -1)).click();
			...
			//By.xpath("//*[@id=\"index\"]/div[2]/div/div/div/div/p"))
			WebElement ventanaProducto = espera.until(ExpectedConditions.visibilityOf(repoHomePage.ventanaPopUpElement()));
			
			//Comparaciones
			Assert.assertEquals(repoHomePage.ventanaPopUpElement().getText(),"Impossible to add the product to the cart.");
	...
	public void verificarCheckout() {
		...
		//SHOPPING-CART SUMMARY queremos recuperar ese texto
		primerTitulo = primerTitulo.substring(0,21);
		Assert.assertEquals(primerTitulo, "SHOPPING-CART SUMMARY");
		Assert.assertEquals(navegador.findElement(By.xpath("//*[@id=\"total_price\"]")).getText(), totalEnString);
		
		navegador.findElement(By.linkText("Proceed to checkout")).click();
		Assert.assertEquals(navegador.getTitle(), "Login - My Shop");
	}
